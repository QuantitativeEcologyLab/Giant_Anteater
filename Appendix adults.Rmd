...
# overlap1_df <- readRDS("rds/overlap1_df.rds")
# overlap2_df <- readRDS("rds/overlap2_df.rds")
# overlap_data <- readRDS("rds/overlap_data.rds")
# #load proximity data
# proximity_data <- readRDS("rds/proximity_data.rds")
# #load distance data
# distance_data <- readRDS("data/rds/distance_data.rds")
# #load encounter data
...
```{r subset-import-meta, message = FALSE}
...
#saveRDS(anteater_data, file = "data/anteater_data_adults.rds")
...
#calculate the mean home range size (est) and CIs (low, high)
meta(AKDE)
```


```{r hr-size}
#extract home range area for each individual and create a dataframe to store the results
...
```{r eval=FALSE, echo=FALSE}
# saveRDS(HR_size, file = "rds/HR_size_adults.rds")
```
...
overlap_data1 <- overlap_result1[[1]]
for (i in 2:num_matrices1) {
  overlap_data1 <- left_join(overlap_data1, overlap_result1[[i]],
...
overlap_data2 <- overlap_result2[[1]]
for (i in 2:num_matrices2) {
  overlap_data2 <- left_join(overlap_data2, overlap_result2[[i]],
                           by = c("anteater_A", "anteater_B"))
}

#combine both sites into one dataframe
overlap_data <- rbind(overlap_data1, overlap_data2)
```

```{r}
#clean up environment
rm(matrix_layer1, matrix_layer2, matrix_level, 
   num_matrices1, num_matrices2,
...
overlap_data <- left_join(overlap_data, rename(bio_data, anteater_A = ID), by = "anteater_A")
overlap_data <- left_join(overlap_data, rename(bio_data, anteater_B = ID), by = "anteater_B", suffix = c(".A", ".B"))

# Add column to indicate which sexes are being compared
overlap_data <- mutate(
  overlap_data, sex_comparison =
...
```{r}
#create a results dataframe for individuals that have an overlap in home-range (dyads that do not have a home-range overlap are not included, ie. 0 value overlap in home-range)
#remove duplicate columns
overlap_data <- overlap_data[-c(7,12,14,15)]

overlap_data <- relocate(overlap_data, c(overlap_low, overlap_est, overlap_high), .after = sex_comparison)
overlap_data <- relocate(overlap_data, c(Road.A, Site.A), .after = anteater_B)
names(overlap_data)[3] <- "Road"
names(overlap_data)[4] <- "Site"
```
...
# saveRDS(object = overlap2_df, file = "rds/overlap1_df.rds")
# saveRDS(object = overlap2_df, file = "rds/overlap2_df.rds")
# saveRDS(object = overlap_data, file = "rds/overlap_data.rds")
```
...
round(mean(overlap_data$overlap_est), 2)

#calculate range of total home range overlap 
round(min(overlap_data$overlap_est), 2)
round(max(overlap_data$overlap_est), 2)
...
```{r}
# for those that have a home-range overlap
#rescale the values
...
overlap_data <- relocate(overlap_data, overlap_est_squeezed, .after = overlap_high)
...
FIT_1 <- FIT[c("Alexander", "Bumpus", "Cate", "Christoffer",
               "Elaine", "Jackson", "Makao", "Puji", "Rodolfo")]
FIT_2 <- FIT[c("Annie", "Beto", "Hannah", "Jane", "Larry",
               "Luigi", "Margaret", "Maria", "Sheron", "Thomas")]
```

```{r eval = FALSE}
#create empty columns for the results to be saved to
overlap1_df$proximity_low <- NA
overlap1_df$proximity_est <- NA
overlap1_df$proximity_high <- NA

#calculate the proximity statistics
for(i in 1:nrow(overlap_1)){
  ANIMAL_A <- as.character(overlap1_df[i, 'anteater_A'])
  ANIMAL_B <- as.character(overlap1_df[i, 'anteater_B'])
  ...
  overlap1_df[i, c("proximity_low")] <- PROXIMITY_1[1]
  overlap1_df[i, c("proximity_est")] <- PROXIMITY_1[2]
  overlap1_df[i, c("proximity_high")] <- PROXIMITY_1[3]
  
  #save results to a csv file
  write.csv(overlap1_df, "data/PROXIMITY1_adults.csv", row.names = FALSE)
  #track progress
...
```{r eval = FALSE}
#create empty columns for the results to be saved to
overlap2_df$proximity_low <- NA
overlap2_df$proximity_est <- NA
overlap2_df$proximity_high <- NA

#calculate the proximity statistics
for(i in 1:nrow(overlap2_df)){
  ANIMAL_A <- as.character(overlap2_df[i, 'anteater_A'])
  ANIMAL_B <- as.character(overlap2_df[i, 'anteater_B'])
  ...
  overlap2_df[i, c("proximity_low")] <- PROXIMITY_2[1]
  overlap2_df[i, c("proximity_est")] <- PROXIMITY_2[2]
  overlap2_df[i, c("proximity_high")] <- PROXIMITY_2[3]
  
  #save results in a csv file
  write.csv(overlap2_df, "data/PROXIMITY2_adults.csv", row.names = FALSE)
  #track progress
  cat("finished index", i, "\n")
}
```
...
PROXIMITY1 <- read_csv("data/PROXIMITY1_adults.csv")
PROXIMITY2 <- read_csv("data/PROXIMITY2_adults.csv")

PROXIMITY2$anteater_A[PROXIMITY2$anteater_A == "Larry 267"] <- "Larry"
PROXIMITY2$anteater_B[PROXIMITY2$anteater_B == "Larry 267"] <- "Larry"

#add missing site column to dataframe for site 2
PROXIMITY2$site <- 2
PROXIMITY2 <- relocate(PROXIMITY2, site, .before = proximity_low)

#create a proximity dataframe
proximity_data <- bind_rows(PROXIMITY1, PROXIMITY2)
proximity_data <- proximity_data[,-3]
proximity_data <- mutate(proximity_data,
                       sex_comparison = 
                         case_when(paste(Sex.A, Sex.B) == "Male Male" ~ "male-male",
                                   paste(Sex.A, Sex.B) == "Female Female" ~ "female-female",
                                   paste(Sex.A, Sex.B) == "Male Female" ~ "female-male",
...
saveRDS(proximity_data, file = "rds/proximity_data.rds")
```


```{r}
#combine overlap and proximity data
overlap_data <- left_join(overlap_data, proximity_data, by = c("anteater_A", "anteater_B",
                                                           "Sex.A", "Sex.B",
                                                           "Age.A", "Age.B",
                                                           "sex_comparison",
                                                           "site"))
...
proximity_test <- glmer(proximity_est ~ sex_comparison + (1|site), 
                        family = Gamma(link = "log"), data = proximity_data)
...
#create empty columns for the results to be saved to
proximity_data$distance_low <- NA
proximity_data$distance_est <- NA
proximity_data$distance_high <- NA
#create empty list for the results to be saved to
RES <- list()

#calculate the distance statistics
for (i in 1:nrow(overlap_data)) {
  ANIMAL_A <- as.character(overlap_data[i, 'anteater_A']) 
  ANIMAL_B <- as.character(overlap_data[i, 'anteater_B'])
...
DISTANCE <- readRDS("data/rds/DATA_DISTANCE.rds")

#locate NA values within the dataframe
DISTANCE[!complete.cases(DISTANCE), ]
...
distance_data <- merge(DISTANCE, overlap_data, by = "pair_ID")
...
saveRDS(distance_data, file = "data/rds/distance_data.rds")
```
...
for(i in 1:length(enc_radius)){
  enc_count[i] <- sum(distance_data$distance_est < enc_radius[i])
}
```
...
for(i in 1:length(enc_radius)){
  ...
  for (j in pair_ID){
    subset_A <- distance_data[distance_data$pair_ID == j,]
...
proximity_data$encounter_count <- NA
pair_ID <- unique(overlap_data$pair_ID)
...
for (i in pair_ID){
  ...
  #save results
  proximity_data[proximity_data$pair_ID == i, "encounter_count"] <- encounter_count
  
}

#number of pairs that had 0 encounters
proximity_data[proximity_data$encounter_count == 0,]
#number of pairs that had at least 1 encounter
proximity_data[proximity_data$encounter_count != 0,]

#calculate the number of encounters based on threshold of 15m
sum(proximity_data$encounter_count)
sum(proximity_data$encounter_count[proximity_data$sex_comparison == "male-male"])
sum(proximity_data$encounter_count[proximity_data$sex_comparison == "female-female"])
sum(proximity_data$encounter_count[proximity_data$sex_comparison == "female-male"])
...
encounter_test <- glmer(encounter_count ~ overlap_est + sex_comparison + (1|site), 
                        family = poisson(link = "log"), 
                        data = proximity_data, subset = encounter_count > 0)
...
proximity_data2 <- proximity_data[proximity_data$encounter_count != 0,]

figure3b_encounters <-
  ggplot(data = proximity_data2,
         aes(y = encounter_count, x = overlap_est)) +
  geom_point(data = proximity_data2, 
...
distance_data$encounter <- ifelse(distance_data$distance_est > 15, 0,1)
encounter_df <- distance_data[which(distance_data$encounter == 1),]
encounter_df$doy <- yday(encounter_df$timestamp)
encs <- aggregate(encounter ~ sex_comparison + doy + pair_ID, 
                  data = encounter_df, FUN = "sum")
encs <- merge(encs, proximity_data[,c("pair_ID","sex_comparison", "site")])
