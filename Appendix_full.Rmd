...
pairwise_1 <- overlap(AKDE_1, level = 0.95)

#extract CI 'est' matrix from array
pairwise_1_est <- pairwise_1$CI[ , , 2]
#remove duplicate values of the matrix
pairwise_1_est[upper.tri(pairwise_1_est, diag = TRUE)] <- NA
#Create a new data frame based on the overlap values
pairwise_1_df <- as.data.frame(pairwise_1_est)
pairwise_1_df$anteater_A <- rownames(pairwise_1_df)
pairwise_1_df <- pivot_longer(pairwise_1_df, cols = -anteater_A, names_to = 'anteater_B', values_to = 'overlap_est', values_drop_na = TRUE)

#extract CI 'low' matrix from array
pairwise_1_low <- pairwise_1$CI[ , , 1]
#remove duplicate values of the matrix
pairwise_1_low[upper.tri(pairwise_1_low, diag = TRUE)] <- NA
#Create a new data frame based on the overlap values
pairwise_1_low <- as.data.frame(pairwise_1_low)
pairwise_1_low$anteater_A <- rownames(pairwise_1_low)
pairwise_1_low <- pivot_longer(pairwise_1_low, cols = -anteater_A, names_to = 'anteater_B', values_to = 'overlap_low', values_drop_na = TRUE)
pairwise_1_df <- left_join(pairwise_1_df, pairwise_1_low, by = c("anteater_A", "anteater_B"))
pairwise_1_df <- relocate(pairwise_1_df, overlap_low, .before = overlap_est)

#extract CI 'high' matrix from array
pairwise_1_high <- pairwise_1$CI[ , , 3]
#remove duplicate values of the matrix
pairwise_1_high[upper.tri(pairwise_1_high, diag = TRUE)] <- NA
#Create a new data frame based on the overlap values
pairwise_1_high <- as.data.frame(pairwise_1_high)
pairwise_1_high$anteater_A <- rownames(pairwise_1_high)
pairwise_1_high <- pivot_longer(pairwise_1_high, cols = -anteater_A, names_to = 'anteater_B', values_to = 'overlap_high', values_drop_na = TRUE)
pairwise_1_df <- left_join(pairwise_1_df, pairwise_1_high, by = c("anteater_A", "anteater_B"))

#add biological data to dataframe
pairwise_1_df <- left_join(pairwise_1_df, rename(DATA_BIO, anteater_A = ID), by = "anteater_A")
pairwise_1_df <- left_join(pairwise_1_df, rename(DATA_BIO, anteater_B = ID), by = "anteater_B", suffix = c(".A", ".B"))
#add column to indicate which sexes that are being compared
pairwise_1_df <- mutate(pairwise_1_df,
...
#assign site
pairwise_1_df$site <- 1
pairwise_1_df <- relocate(pairwise_1_df, c("overlap_low", "overlap_est", "overlap_high"), .after = site)

#Create a pairwise data frame pairing up every individual for site  ...................
#calculate 95% AKDE overlap for pairwise comparison
pairwise_2 <- overlap(AKDE_2, level = 0.95)

#extract CI 'est' matrix from array
pairwise_2_est <- pairwise_2$CI[ , , 2]
#remove duplicate values of the matrix
pairwise_2_est[upper.tri(pairwise_2_est, diag = TRUE)] <- NA
#Create a new data frame based on the overlap values
pairwise_2_df <- as.data.frame(pairwise_2_est)
pairwise_2_df$anteater_A <- rownames(pairwise_2_df)
pairwise_2_df <- pivot_longer(pairwise_2_df, cols = -anteater_A, names_to = 'anteater_B', values_to = 'overlap_est', values_drop_na = TRUE)

#extract CI 'low' matrix from array
pairwise_2_low <- pairwise_2$CI[ , , 1]
#remove duplicate values of the matrix
pairwise_2_low[upper.tri(pairwise_2_low, diag = TRUE)] <- NA
#Create a new data frame based on the overlap values
pairwise_2_low <- as.data.frame(pairwise_2_low)
pairwise_2_low$anteater_A <- rownames(pairwise_2_low)
pairwise_2_low <- pivot_longer(pairwise_2_low, cols = -anteater_A, names_to = 'anteater_B', values_to = 'overlap_low', values_drop_na = TRUE)
pairwise_2_df <- left_join(pairwise_2_df, pairwise_2_low, by = c("anteater_A", "anteater_B"))
pairwise_2_df <- relocate(pairwise_2_df, overlap_low, .before = overlap_est)

#extract CI 'high' matrix from array
pairwise_2_high <- pairwise_2$CI[ , , 3]
#remove duplicate values of the matrix
pairwise_2_high[upper.tri(pairwise_2_high, diag = TRUE)] <- NA
#Create a new data frame based on the overlap values
pairwise_2_high <- as.data.frame(pairwise_2_high)
pairwise_2_high$anteater_A <- rownames(pairwise_2_high)
pairwise_2_high <- pivot_longer(pairwise_2_high, cols = -anteater_A, names_to = 'anteater_B', values_to = 'overlap_high', values_drop_na = TRUE)
pairwise_2_df <- left_join(pairwise_2_df, pairwise_2_high, by = c("anteater_A", "anteater_B"))

#add biological data to dataframe
pairwise_2_df <- left_join(pairwise_2_df, rename(DATA_BIO, anteater_A = ID), by = "anteater_A")
pairwise_2_df <- left_join(pairwise_2_df, rename(DATA_BIO, anteater_B = ID), by = "anteater_B", suffix = c(".A", ".B"))
#add column to indicate which sexes that are being compared
pairwise_2_df <- mutate(pairwise_2_df,
                        sex_comparison = case_when(paste(Sex.A, Sex.B) == "Male Male" ~ "male-male",
                                                   paste(Sex.A, Sex.B) == "Female Female" ~ "female-female",
                                                   paste(Sex.A, Sex.B) == "Male Female" ~ "male-female",
                                                   paste(Sex.A, Sex.B) == "Female Male" ~ "male-female"))
#assign site
pairwise_2_df$site <- 2
pairwise_2_df <- relocate(pairwise_2_df, c("overlap_low", "overlap_est", "overlap_high"), .after = site)
## HRO pairwise results
pairwise_df <- rbind(pairwise_1_df, pairwise_2_df)

saveRDS(object = pairwise_1_df, file = "RDS/pairwise_1_df.RDS")
saveRDS(object = pairwise_2_df, file = "RDS/pairwise_2_df.RDS")
saveRDS(object = pairwise_df, file = "RDS/pairwise_df.RDS")
```
...
pairwise_1_df$proximity_low <- NA
pairwise_1_df$proximity_est <- NA
pairwise_1_df$proximity_high <- NA
#Calculate the proximity statistics
for(i in 1:nrow(overlap_1)){
  ANIMAL_A <- as.character(pairwise_1_df[i, 'anteater_A']) # add as.character due to tibble format
  ANIMAL_B <- as.character(pairwise_1_df[i, 'anteater_B'])
  ...
  pairwise_1_df[i, c("proximity_low")] <- PROXIMITY_1[1]
  pairwise_1_df[i, c("proximity_est")] <- PROXIMITY_1[2]
  pairwise_1_df[i, c("proximity_high")] <- PROXIMITY_1[3]
  write.csv(pairwise_1_df, "data/DATA_proximity_1.csv", row.names = FALSE)
  cat("finished index", i, "\n") # see the loop happening in real time
}

#create empty columns for results to be saved to
pairwise_2_df$proximity_low <- NA
pairwise_2_df$proximity_est <- NA
pairwise_2_df$proximity_high <- NA
#Calculate the proximity statistics
for(i in 1:nrow(pairwise_2_df)){
  ANIMAL_A <- as.character(pairwise_2_df[i, 'anteater_A']) # add as.character due to tibble format
  ANIMAL_B <- as.character(pairwise_2_df[i, 'anteater_B'])
  ...
  pairwise_2_df[i, c("proximity_low")] <- PROXIMITY_2[1]
  pairwise_2_df[i, c("proximity_est")] <- PROXIMITY_2[2]
  pairwise_2_df[i, c("proximity_high")] <- PROXIMITY_2[3]
  write.csv(pairwise_2_df, "data/DATA_proximity_2.csv", row.names = FALSE)
  cat("finished index", i, "\n") # see the loop happening in real time
}
...
cd_pair1 <- as.corrData(pair1_GPS)
...
#Create corrData object.
cd_pair2 <- as.corrData(pair2_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair3 <- as.corrData(pair3_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair4 <- as.corrData(pair4_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair5 <- as.corrData(pair5_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair6 <- as.corrData(pair6_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair7 <- as.corrData(pair7_GPS)
#Estimate the partition points 
...
#Create corrData object.
cd_pair8 <- as.corrData(pair8_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair9 <- as.corrData(pair9_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair10 <- as.corrData(pair10_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair11 <- as.corrData(pair11_GPS)
#Estimate the partition points
...
#Create corrData object.
cd_pair12 <- as.corrData(pair12_GPS)
#Estimate the partition points
prts_pair12 <- findPrts(cd_pair12, W=5, IC = 2)
#Get the MCI estimates and selected model conditional on the data and partition points
cm_pair12 <- corrMove(cd_pair12, prts_pair12)

saveRDS(cm_pair1, file = "RDS/cmAnteater_pair1.RDS")
saveRDS(cm_pair2, file = "RDS/cmAnteater_pair2.RDS")
saveRDS(cm_pair3, file = "RDS/cmAnteater_pair3.RDS")
saveRDS(cm_pair4, file = "RDS/cmAnteater_pair4.RDS")
saveRDS(cm_pair5, file = "RDS/cmAnteater_pair5.RDS")
saveRDS(cm_pair6, file = "RDS/cmAnteater_pair6.RDS")
saveRDS(cm_pair7, file = "RDS/cmAnteater_pair7.RDS")
saveRDS(cm_pair8, file = "RDS/cmAnteater_pair8.RDS")
saveRDS(cm_pair9, file = "RDS/cmAnteater_pair9.RDS")
saveRDS(cm_pair10, file = "RDS/cmAnteater_pair10.RDS")
saveRDS(cm_pair11, file = "RDS/cmAnteater_pair11.RDS")
saveRDS(cm_pair12, file = "RDS/cmAnteater_pair12.RDS")
```
