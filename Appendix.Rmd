...
Our primary aim was to understand the socio-spatial ecology of giant anteater (*Myrmecophaga tridactyla*). We aimed to address two overarching questions: 
...
To do this, we used GPS data to quantify key movement metrics and social association using the methods implemented in the ```ctmm``` package and the```corrMove``` package.
...
*UPDATE SECTION reorganizing -> In this appendix, we detail the workflow that was used on data that were collected as a part of a broader giant anteater monitoring program. Additionally, we describe in detail the analyses of the movement metrics and the interactions and associations. The results were compiled and analyzed as described in the main text.*
...
setwd("C:/Users/Kat/Documents/GitHub/Giant_Anteater")
# Load data
...
TELEMETRY_FULL <- as.telemetry(DATA_GPS)

#visualisation of the full dataset
plot(TELEMETRY_FULL)
```
